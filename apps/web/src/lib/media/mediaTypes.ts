...
export type LoadedMedia = {
  ...
  kind: MediaKind;
  format: MediaFormat;
  origin: "local-file" | "remote-url" | "remote-stream";
  sizeBytes?: number;
  durationSecs?: number;
  modifiedMs?: number;
};
...
export function inferMediaFormat(typeOrUrl: string): MediaFormat {
  ...
  if (
    ...
    /\.(mp4|webm|mkv|mov|m4v|ogv|mp3|wav|aac|m4a|flac|ogg|opus|ts|m2ts|mpegts)(\?|#|$)/.test(value)
  ) {
    return "direct";
  }
...
export function inferMediaKind(typeOrUrl: string): MediaKind {
  ...
  if (
    ...
    value.startsWith("video/") ||
    /\.(m3u8|mpd|mp4|webm|mkv|mov|m4v|ogv|ts|m2ts)(\?|#|$)/.test(value)
  ) {
    return "video";
  }
